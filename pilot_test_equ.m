...
%         if i == 1
%             if abs(tx_signal_matrix(i,j)) == 0
%                 h(i,j) = 1; % 防止因为除0导致NaN出现
%             else
%                 h(i,j) = faded_signal_matrix(i,j) ./ tx_signal_matrix(i,j);
%             end
%         elseif mod(i,pilot_spacing) == 1
%             if abs(tx_signal_matrix(i,j)) == 0
%                 h(i,j) = 1;
%             else
%                 h(i,j) = faded_signal_matrix(i,j) ./ tx_signal_matrix(i,j);
...
% % 对导频之间的子载波进行频域线性插值
% pilot_index_row = 1:pilot_spacing:size(tx_signal_matrix,1);
% for t = 1:size(tx_signal_matrix,2)
%     h(:,t) = interp1(pilot_index_row,h(pilot_index_row,t),1:size(tx_signal_matrix,1),"linear",1);
% end
...
% 对时域再进行插值
pilot_index_column = 1:pilot_spacing:size(tx_signal_matrix,2);
for r = 1:size(tx_signal_matrix,1)
    if mod(r,pilot_spacing) == 0
        % 导频序列不插值
        continue;
    end
    h(r,:) = interp1(pilot_index_column,h(r,pilot_index_column),1:size(tx_signal_matrix,2));
end
