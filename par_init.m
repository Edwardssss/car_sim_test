...
sim_struct.ofdm_subframe_num = sim_struct.M;              % OFDM subsymbol number
sim_struct.k_bit = log2(sim_struct.mod_level);            % number of bits per symbol
...
sim_struct.cp_len = floor(sim_struct.cp_size * sim_struct.N);        % cyclic prefix length
...
sim_struct.data_len = sim_struct.N - sim_struct.guard_size;          % information bit length
sim_struct.intr_seed = 4831;                   % random interleaving seed
sim_struct.B = sim_struct.N * sim_struct.df;                         % signal bandwidth
sim_struct.max_v = 40;                         % maximum relative velocity(m/s)
sim_struct.c = physconst("LightSpeed");        % speed of light
sim_struct.max_doppler_shift = sim_struct.max_v * sim_struct.fc / sim_struct.c; % maximum doppler shift
sim_struct.range_min = sim_struct.c / (sim_struct.N * sim_struct.df);           % range resolution(m)
sim_struct.doppler_min = sim_struct.df / sim_struct.M;               % doppler shift resolution(Hz)
sim_struct.v_min = sim_struct.doppler_min * sim_struct.c / sim_struct.fc;       % velocity resolution(m/s)
sim_struct.otfs_frame_time = sim_struct.M / sim_struct.df;           % one frame length(s)
sim_struct.chan_time = 1 / (sim_struct.max_v * sim_struct.fc / sim_struct.c);   % channel coherence time(s)
...
sim_struct.snr = sim_struct.EbN0_dB + 10 * log10(...
    sim_struct.code_rate * sim_struct.k_bit) + 10 * ...
    log10(sim_struct.data_len / sim_struct.N);            % snr
%% Output Result
